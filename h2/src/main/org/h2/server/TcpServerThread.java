...
public class TcpServerThread implements Runnable {
        ...
        try {
            ...
            try {
                ...
                if (minClientVersion < 6) {
                    throw DbException.get(ErrorCode.DRIVER_VERSION_ERROR_2, Integer.toString(minClientVersion), "" + Constants.TCP_PROTOCOL_VERSION_MIN_SUPPORTED);
                }

                int maxClientVersion = transfer.readInt();
                if (maxClientVersion < Constants.TCP_PROTOCOL_VERSION_MIN_SUPPORTED) {
                    throw DbException.get(ErrorCode.DRIVER_VERSION_ERROR_2, Integer.toString(maxClientVersion), "" + Constants.TCP_PROTOCOL_VERSION_MIN_SUPPORTED);
                }

                if (minClientVersion > Constants.TCP_PROTOCOL_VERSION_MAX_SUPPORTED) {
                    throw DbException.get(ErrorCode.DRIVER_VERSION_ERROR_2, Integer.toString(minClientVersion), "" + Constants.TCP_PROTOCOL_VERSION_MAX_SUPPORTED);
                }

                clientVersion = Math.min(maxClientVersion, Constants.TCP_PROTOCOL_VERSION_MAX_SUPPORTED);
                ...
                ConnectionInfo connectionInfo = new ConnectionInfo(db);
                connectionInfo.setOriginalURL(originalURL);
                connectionInfo.setUserName(transfer.readString());
                connectionInfo.setUserPasswordHash(transfer.readBytes());
                connectionInfo.setFilePasswordHash(transfer.readBytes());
                int len = transfer.readInt();
                for (int i = 0; i < len; i++) {
                    connectionInfo.setProperty(transfer.readString(), transfer.readString());
                }
                ...
                transfer.flush();
                if (connectionInfo.filePasswordHash != null) {
                    connectionInfo.setFileEncryptionKey(transfer.readBytes());
                }
                ...
                server.addConnection(threadId, originalURL, connectionInfo.getUserName());
                trace("Connected");
    ...
    private void sendError(Throwable t, boolean withStatus) {
        try {
            SQLException e = DbException.convert2DbException(t).getSQLException();
