...
public class MVStore implements AutoCloseable {
     ...
     */
    private CacheLongKeyLIRS<long[]> chunkTocCache;
    ...
    private final ConcurrentHashMap<Integer, Chunk> chunkId_chunk = new ConcurrentHashMap<>();

    /**
     * 来源 mvStore 的 accountForRemovedPage() <br>
     * 用途 mvStore 的 acceptChunkOccupancyChanges()
     */
    private final Queue<RemovedPageInfo> removedPages = new PriorityBlockingQueue<>();

    /**
     * 来源 mvStore 的 acceptChunkOccupancyChanges() <br>
     * 用途 mvStore 的 dropUnusedChunks()
     */
    ...
    /**
     * name.map名 -> mapId的hexString
     */
    private final MVMap<String, String> meta;

    private final ConcurrentHashMap<Integer, MVMap<?, ?>> mvMapId_mvMap = new ConcurrentHashMap<>();
     ...
     * instead, the unused field is set. If smaller zero, chunks are directly overwritten if unused.
     */
    ...
    MVStore(Map<String, Object> config) {
        ...
        if ((FileStore) config.get("fileStore") == null) {
            if (fileName != null) {
    ...
    private MVMap<String, String> openMetaMap() {
        ...
        MVMap<String, String> mvMap = new MVMap<>(this, metaId, StringDataType.INSTANCE, StringDataType.INSTANCE);
        mvMap.setRootPos(getRootPos(mvMap.getId()), currentVersion - 1);
        return mvMap;
    }
     ...
     * does not yet exist. If a map with this name is already open, this map is returned.
     *
    ...
    public <M extends MVMap<K, V>, K, V> M openMap(String name, MVMap.MapBuilder<M, K, V> builder) {
        // DataUtils.META_NAME + name -> int的id
        int mvMapId = getMapId(name);

        // 当db启动时候还原已创建的表的时候id是比0大的
        if (mvMapId >= 0) {
            M mvMap = (M) getMap(mvMapId);
            if (mvMap == null) {
                mvMap = openMap(mvMapId, builder);
            }

            assert builder.getKeyType() == null || mvMap.getKeyType().getClass().equals(builder.getKeyType().getClass());
            assert builder.getValueType() == null || mvMap.getValueType().getClass().equals(builder.getValueType().getClass());

            return mvMap;
        }

        mvMapId = lastMapId.incrementAndGet();
        assert getMap(mvMapId) == null;

        HashMap<String, Object> map = new HashMap<>();
        map.put("id", mvMapId);
        map.put("createVersion", currentVersion);
        M mvMap = builder.create(this, map);

        String mapIdHexString = Integer.toHexString(mvMapId);

        // DataUtils.META_NAME + hex的id ->
        meta.put(MVMap.getMapKey(mvMapId), mvMap.asString(name));

        // 对应 getMapId() DataUtils.META_NAME + name -> hex的id
        String existing = meta.putIfAbsent(DataUtils.META_NAME + name, mapIdHexString);
        if (existing != null) { // looks like map was created concurrently, cleanup and re-start
            meta.remove(MVMap.getMapKey(mvMapId));
            return openMap(name, builder);
        }

        mvMap.setRootPos(0, currentVersion - 1);

        markMetaChanged();

        M existingMap = (M) mvMapId_mvMap.putIfAbsent(mvMapId, mvMap);
        if (existingMap != null) {
            mvMap = existingMap;
        }
    ...
    public <M extends MVMap<K, V>, K, V> M openMap(int id, MVMap.MapBuilder<M, K, V> builder) {
        M mvMap;
        while ((mvMap = (M) getMap(id)) == null) { // name.table.1 -> 7 然后 map.7 -> name:table.1,createVersion:1,key:8fa25204,val:5eb3a958
            ...
            config.put("id", id);
            mvMap = builder.create(this, config); // txMapBuilder
            long root = getRootPos(id);
            long lastStoredVersion = currentVersion - 1;
            mvMap.setRootPos(root, lastStoredVersion);
            if (mvMapId_mvMap.putIfAbsent(id, mvMap) == null) {
                break;
            }
    ...
    public <K, V> MVMap<K, V> getMap(int id) {
        checkOpen();
        @SuppressWarnings("unchecked")
        MVMap<K, V> map = (MVMap<K, V>) mvMapId_mvMap.get(id);
        return map;
    }
    ...
    private MVMap<String, String> getLayoutMap(long version) {
        ...
        long block = c.startBlockNumInFile;
    ...
    private Chunk getChunkForVersion(long version) {
        Chunk newest = null;
        for (Chunk c : chunkId_chunk.values()) {
    ...
    private void readStoreHeader() {
        ...
        Comparator<Chunk> chunkComparator = (one, two) -> {
            ...
            if (result == 0) {
                ...
                result = Long.compare(one.startBlockNumInFile, two.startBlockNumInFile);
            }
            return result;
        };
        ...
        if (!assumeCleanShutdown) {
            ...
            if (tailChunk != null) {
                blocksInStore = tailChunk.startBlockNumInFile; // for a possible full scan later on
            ...
            if (newest != null) {
                ...
                while (true) {
                    validChunksByLocation.put(newest.startBlockNumInFile, newest);
                    if (newest.nextChunkPosition == 0 || newest.nextChunkPosition >= blocksInStore) {
                        // no (valid) next
                        break;
                    }
                    Chunk test = readChunkHeaderAndFooter(newest.nextChunkPosition, newest.id + 1);
        ...
        if (assumeCleanShutdown) {
            ...
            try {
                ...
                while (cursor.hasNext() && cursor.next().startsWith(DataUtils.META_CHUNK)) {
                    ...
                    chunkId_chunk.putIfAbsent(c.id, c);
                ...
                while (assumeCleanShutdown && (c = chunksToVerify.poll()) != null) {
                    Chunk test = readChunkHeaderAndFooter(c.startBlockNumInFile, c.id);
                    assumeCleanShutdown = test != null;
                    if (assumeCleanShutdown) {
                        validChunksByLocation.put(test.startBlockNumInFile, test);
                    }
                }
        ...
        if (!assumeCleanShutdown) {
            ...
            if (!quickRecovery) {
                ...
                while ((tailChunk = discoverChunk(block)) != null) {
                    block = tailChunk.startBlockNumInFile;
        ...
        for (Chunk c : chunkId_chunk.values()) {
            if (c.isSaved()) {
                long start = c.startBlockNumInFile * BLOCK_SIZE;
        ...
        for (Chunk chunk : lastChunkCandidates) {
            ...
            try {
                ...
                while (cursor.hasNext() && cursor.next().startsWith(DataUtils.META_CHUNK)) {
                    ...
                    // might be there already, due to meta traversal
                    // see readPage() ... getChunkIfFound()
                    Chunk test = chunkId_chunk.putIfAbsent(c.id, c);
                    if (test != null) {
                        c = test;
                    }
                    assert chunkId_chunk.get(c.id) == c;
                    if ((test = validChunksByLocation.get(c.startBlockNumInFile)) == null || test.id != c.id) {
                        if ((test = validChunksById.get(c.id)) != null) {
                            ...
                            // location instead.
                            c.startBlockNumInFile = test.startBlockNumInFile;
                        } else if (c.isLive() && (afterFullScan || readChunkHeaderAndFooter(c.startBlockNumInFile, c.id) == null)) {
                    ...
                    if (!c.isLive()) {
                        ...
                        c.startBlockNumInFile = Long.MAX_VALUE;
                        c.blockCount = Integer.MAX_VALUE;
                        if (c.noLongerNeededTime == 0) {
                            c.noLongerNeededTime = creationTime;
                        }
    ...
    private void setLastChunk(Chunk lastChunk) {
        chunkId_chunk.clear();
        ...
        int mapId = 0;
        if (lastChunk != null) { // there is a valid chunk
            lastChunkId = lastChunk.id;
            currentVersion = lastChunk.version;
            layoutRootPos = lastChunk.layoutRootPos;
            mapId = lastChunk.mapId;
            chunkId_chunk.put(lastChunk.id, lastChunk);
        }
        lastMapId.set(mapId);
    ...
    private Chunk discoverChunk(long block) {
        ...
        while (true) {
            ...
            if (test != null) {
                ...
                test = readChunkHeaderOptionally(test.startBlockNumInFile, test.id);
                if (test != null) {
                    // if that footer has a corresponding header,
                    // consider them as a new candidate for a valid chunk
                    candidate = test;
                    candidateLocation = test.startBlockNumInFile;
                }
            }
    ...
    private Chunk readChunkHeaderAndFooter(long block, int expectedId) {
        ...
        if (header != null) {
            Chunk footer = readChunkFooter(block + header.blockCount);
            if (footer == null || footer.id != expectedId || footer.startBlockNumInFile != header.startBlockNumInFile) {
                return null;
            }
        }
        return header;
    ...
    private void writeMVStoreHeader() {
        Chunk lastChunk = this.lastChunk;
        if (lastChunk != null) {
            storeHeader.put(HDR_BLOCK, lastChunk.startBlockNumInFile);
        ...
        DataUtils.appendMap(stringBuilder, storeHeader);

        byte[] byteArr = stringBuilder.toString().getBytes(StandardCharsets.ISO_8859_1);

        int checksum = DataUtils.getFletcher32(byteArr, 0, byteArr.length);
        DataUtils.appendMap(stringBuilder, HDR_FLETCHER, checksum);
        stringBuilder.append('\n');
        byteArr = stringBuilder.toString().getBytes(StandardCharsets.ISO_8859_1);
                    ...
                    try {
                        try {
                            if (normalShutdown && fileStore != null && !fileStore.isReadOnly()) {
                                for (MVMap<?, ?> map : mvMapId_mvMap.values()) {
                                    if (map.isClosed()) {
                                ...
                                if (allowedCompactionTime > 0) {
                                    compactUnderTimeLimit(allowedCompactionTime);
                            ...
                            // release memory early - this is important when called because of out of memory
                            clearCaches();

                            for (MVMap<?, ?> mvMap : new ArrayList<>(mvMapId_mvMap.values())) {
                                mvMap.close();
                            }
     ...
     */
    private Chunk getChunk(long position) {
        int chunkId = DataUtils.getPageChunkId(position);

        Chunk chunk = chunkId_chunk.get(chunkId);
        if (chunk == null) {
            checkOpen();
            ...
            chunk = Chunk.fromString(s);
            if (!chunk.isSaved()) {
                throw DataUtils.newMVStoreException(DataUtils.ERROR_FILE_CORRUPT, "Chunk {0} is invalid", chunkId);
            }

            chunkId_chunk.put(chunk.id, chunk);
        }
    ...
    private void setWriteVersion(long version) {
        for (Iterator<MVMap<?, ?>> iter = mvMapId_mvMap.values().iterator(); iter.hasNext(); ) {
            MVMap<?, ?> map = iter.next();
     ...
     * otherwise it increments the current version and stores the data (for file based stores).
     * <p>
     ...
     * 其实返回的未用到
     */
    private long commit(Predicate<MVStore> needStore) {
        // we need to prevent re-entrance, which may be possible,
        // because meta map is modified within storeNow() and that
        // causes beforeWrite() call with possibility of going back here
                ...
                try {
                    ...
                    if (fileStore == null) {
                        ...
                        if (fileStore.readOnly) {
                            throw DataUtils.newMVStoreException(DataUtils.ERROR_WRITING_FAILED, "This store is read-only");
                        }

                        storeNow(syncWrite, 0, () -> reuseSpace ? 0 : getLastFreeBlockNum());
                    }
                } finally {  // in any case reset the current store version, to allow closing the store
    ...
    private void storeNow(boolean sync, long reservedLow, Supplier<Long> reservedHighSupplier) {
        try {
            lastCommitTime = getDurationSinceCreation();
            ...
            // noinspection NonAtomicOperationOnVolatileField
            long newCurrentVersion = ++currentVersion;

            ArrayList<Page<?, ?>> changedRootPageList = collectChangedMvMapRootPage(newCurrentVersion);

            assert storeLock.isHeldByCurrentThread();

            submitOrRun(serializationExecutor,
                    () -> serializeAndStore(sync, reservedLow, reservedHighSupplier, changedRootPageList, lastCommitTime, newCurrentVersion), sync);
    ...
    private static void submitOrRun(ThreadPoolExecutor executor, Runnable action, boolean sync) throws ExecutionException {
        if (executor != null) {
            try {
                Future<?> future = executor.submit(action);

                if (sync || executor.getQueue().size() > PIPE_LENGTH) {
                    try {
                        future.get();
    ...
    // 内部调用mvMap的setWriteVersion
    private ArrayList<Page<?, ?>> collectChangedMvMapRootPage(long newCurrentVersion) {
        // 因为 currentStoredVersion 是 newCurrentVersion-1
        long lastStoredVersion = newCurrentVersion - 2;

        ArrayList<Page<?, ?>> changedRootPageList = new ArrayList<>();

        // mvMapId_mvMap 体系
        for (Iterator<MVMap<?, ?>> iterator = mvMapId_mvMap.values().iterator(); iterator.hasNext(); ) {
            MVMap<?, ?> mvMap = iterator.next();

            RootReference<?, ?> rootReference = mvMap.setWriteVersion(newCurrentVersion);

            if (rootReference == null) {
                iterator.remove();
            } else if (mvMap.getCreateVersion() < newCurrentVersion &&
                    !mvMap.isVolatile && mvMap.hasChangesSince(lastStoredVersion)) { // if map was created after storing started, skip it
                assert rootReference.version <= newCurrentVersion : rootReference.version + " > " + newCurrentVersion;

                Page<?, ?> rootPage = rootReference.rootPage;

                // after deletion previously saved leaf may pop up as a root, but we still need to save new root pos in meta
                if (!rootPage.isSaved() || rootPage.isLeaf()) {
                    changedRootPageList.add(rootPage);
                }
            }
        }

        // meta 体系
        RootReference<?, ?> rootReference = meta.setWriteVersion(newCurrentVersion);
        if (meta.hasChangesSince(lastStoredVersion) || metaChanged) {
            assert rootReference != null && rootReference.version <= newCurrentVersion : rootReference == null ? "null" : rootReference.version + " > " + newCurrentVersion;
        ...
        return changedRootPageList;
    }

    private void serializeAndStore(boolean sync,
                                   long reservedLow,
                                   Supplier<Long> reservedHighSupplier,
                                   ArrayList<Page<?, ?>> changedRootPageList,
                                   long time,
                                   long version) {
        ...
        try {
            ...
            serialize2WriteBuffer(writeBuffer, changedRootPageList, chunk, reservedLow, reservedHighSupplier);

            submitOrRun(bufferSaveExecutor, () -> storeBuffer(chunk, writeBuffer, changedRootPageList), sync);
        } catch (MVStoreException e) {
            panic(e);
        } catch (Throwable e) {
            panic(DataUtils.newMVStoreException(DataUtils.ERROR_INTERNAL, "{0}", e.toString(), e));
    ...
    private Chunk createChunk(long time, long version) {
        int lastChunkId0 = lastChunkId;

        if (lastChunkId0 != 0) {
            lastChunkId0 &= Chunk.MAX_ID;
            Chunk lastChunk = chunkId_chunk.get(lastChunkId0);
            ...
            layout.put(Chunk.getMetaKey(lastChunkId0), lastChunk.asString());

            // never go backward in time
            time = Math.max(lastChunk.creationTime, time);
        }
        ...
        while (true) {
            ...
            Chunk old = chunkId_chunk.get(newChunkId);
            if (old == null) {
                break;
            }

            if (!old.isSaved()) {
                MVStoreException e = DataUtils.newMVStoreException(DataUtils.ERROR_INTERNAL, "Last block {0} not stored, possibly due to out-of-memory", old);
                panic(e);
            }
        }

        Chunk chunk = new Chunk(newChunkId);
        chunk.pageCount = 0;
        chunk.pageCountLive = 0;
        chunk.maxLen = 0;
        chunk.maxLenLive = 0;
        chunk.layoutRootPos = Long.MAX_VALUE;
        chunk.startBlockNumInFile = Long.MAX_VALUE;
        chunk.blockCount = Integer.MAX_VALUE;
        chunk.creationTime = time;
        chunk.version = version;
        chunk.nextChunkPosition = Long.MAX_VALUE;
        chunk.occupancy = new BitSet();

        return chunk;
    }

    private void serialize2WriteBuffer(WriteBuffer writeBuffer,
                                       ArrayList<Page<?, ?>> changedRootPageList,
                                       Chunk chunk,
                                       long reservedLow,
                                       Supplier<Long> reservedHighSupplier) {
        ...
        int headerLength = writeBuffer.position() + 44;
        writeBuffer.position(headerLength);

        long version = chunk.version;

        List<Long> toc = new ArrayList<>();

        for (Page<?, ?> changedRootPage : changedRootPageList) {
            String key = MVMap.getMapRootKey(changedRootPage.mvMap.getId());

            if (changedRootPage.getTotalCount() == 0) {
                layout.remove(key);
            } else {
                changedRootPage.writeUnsavedRecursive(chunk, writeBuffer, toc);
                layout.put(key, Long.toHexString(changedRootPage.position));
            }
        }

        acceptChunkOccupancyChanges(chunk.creationTime, version);
        ...
        Page<String, String> layoutRoot = layoutRootReference.rootPage;
        layoutRoot.writeUnsavedRecursive(chunk, writeBuffer, toc);
        chunk.layoutRootPos = layoutRoot.getPosition();
        changedRootPageList.add(layoutRoot);
        ...
        for (long tocElement : toc) {
            ...
            writeBuffer.putLong(tocElement);
        ...
        chunkTocCache.put(chunk.id, tocArray);
        int chunkLength = writeBuffer.position();
        ...
        try {
            ...
            long filePos = fileStore.freeSpaceBitSet.allocate(writeBuffer.limit(), reservedLow, reservedHigh);
            chunk.blockCount = writeBuffer.limit() / BLOCK_SIZE;
            chunk.startBlockNumInFile = filePos / BLOCK_SIZE;
            assert validateFileLength(chunk.asString());
            // calculate and set the likely next position
            if (reservedLow > 0 || reservedHigh == reservedLow) {
                chunk.nextChunkPosition = fileStore.predictAllocation(chunk.blockCount, 0, 0);
            } else {
                // just after this chunk
                chunk.nextChunkPosition = 0;
            }
            ...
            chunk.writeChunkHeader(writeBuffer, headerLength);

            writeBuffer.position(writeBuffer.limit() - Chunk.FOOTER_LENGTH);
    ...
    private void storeBuffer(Chunk c, WriteBuffer buff, ArrayList<Page<?, ?>> changed) {
        ...
        try {
            ...
            long filePos = c.startBlockNumInFile * BLOCK_SIZE;
    ...
    private boolean isWriteStoreHeader(Chunk c, boolean storeAtEndOfFile) {
        ...
        if (!storeAtEndOfFile) {
            ...
            if (lastChunk == null) {
            ...
            } else if (lastChunk.nextChunkPosition != c.startBlockNumInFile) {
                ...
                if (lastChunk.version - headerVersion > 20) {
                    ...
                    for (int chunkId = DataUtils.readHexInt(storeHeader, HDR_CHUNK, 0);
                        ...
                        // was removed
                        writeStoreHeader = !chunkId_chunk.containsKey(chunkId);
                    }
                }
            }
        }
    ...
    private static boolean canOverwriteChunk(Chunk chunk, long oldestVersionToKeep) {
        return !chunk.isLive() && chunk.unusedAtVersion < oldestVersionToKeep;
    }

    private boolean isSeasonedChunk(Chunk chunk, long time) {
        return retentionTime < 0 || chunk.creationTime + retentionTime <= time;
    }

    private long getDurationSinceCreation() {
     ...
     * their usage is over.
     */
    private void acceptChunkOccupancyChanges(long durationSinceMvStoreCreation, long version) {
        ...
        while (true) {
            ...
            while ((removedPageInfo = removedPages.peek()) != null && removedPageInfo.version < version) {
                // could be different from the peeked one
                removedPageInfo = removedPages.poll();

                // since nobody else retrieves from queue
                assert removedPageInfo != null;
                assert removedPageInfo.version < version : removedPageInfo + " < " + version;

                int chunkId = removedPageInfo.getPageChunkId();
                Chunk chunk = chunkId_chunk.get(chunkId);
                ...
                if (chunk != null) {
                    ...
                    if (chunk.accountForRemovedPage(removedPageInfo.getPageNo(), removedPageInfo.getPageLength(), removedPageInfo.isPinned(), durationSinceMvStoreCreation, removedPageInfo.version)) {
    ...
    private long getFileLengthInUse() {
        ...
        long result = fileStore.freeSpaceBitSet.getLastFreePosition();
     ...
     * @return block index
     */
    private long getLastFreeBlockNum() {
        assert saveChunkLock.isHeldByCurrentThread();
        return fileStore.freeSpaceBitSet.getLastFreeBlockNum();
    }
    ...
    private long measureFileLengthInUse() {
        ...
        for (Chunk c : chunkId_chunk.values()) {
            if (c.isSaved()) {
                size = Math.max(size, c.startBlockNumInFile + c.blockCount);
            }
        }
    ...
    public boolean hasUnsavedChanges() {
        ...
        for (MVMap<?, ?> mvMap : mvMapId_mvMap.values()) {
            if (mvMap.isClosed()) {
                continue;
            }

            if (mvMap.hasChangesSince(lastStoredVersion)) {
                return true;
            }
        }
    ...
    private Chunk readChunkHeaderOptionally(long block) {
        try {
            ...
            return chunk.startBlockNumInFile != block ? null : chunk;
        } catch (Exception ignore) {
            return null;
        }
    }
            ...
            try {
                ...
                try {
                    if (lastChunk != null && reuseSpace && getFillRate() <= targetFillRate) {
                        ...
                        // 尚未真正意义上在文件上干掉chunk对应的内容 只是在 chunkId_chunk chunkCache layout 去掉对应的记录
                        dropUnusedChunks();

                        long blockNumInFile = fileStore.freeSpaceBitSet.getFirstFreePosition() / BLOCK_SIZE;
    ...
    private List<Chunk> findChunksNeedMove(long blockNumInFile, long moveSize) {
        ...
        List<Chunk> chunksNeedMove = new ArrayList<>();

        if (maxBlocksToMove > 0) {
            PriorityQueue<Chunk> queue = new PriorityQueue<>(chunkId_chunk.size() / 2 + 1,
                    (o1, o2) -> {
                        ...
                        return Long.signum(o2.startBlockNumInFile - o1.startBlockNumInFile);
                    });

            long size = 0;

            for (Chunk chunk : chunkId_chunk.values()) {
                if (chunk.isSaved() && chunk.startBlockNumInFile > blockNumInFile) {
                    chunk.collectPriority = fileStore.freeSpaceBitSet.getMovePriority((int) chunk.startBlockNumInFile);
                    queue.offer(chunk);
            ...
            if (!queue.isEmpty()) {
                chunksNeedMove = new ArrayList<>(queue);
                chunksNeedMove.sort(Chunk.PositionComparator.INSTANCE);
            }
        }
    ...
    private boolean moveChunkInside(Chunk chunkToMove, long boundary) {
        boolean res = chunkToMove.startBlockNumInFile >= boundary &&
                fileStore.predictAllocation(chunkToMove.blockCount, boundary, -1) < boundary &&
                moveChunk(chunkToMove, boundary, -1);
        assert !res || chunkToMove.startBlockNumInFile + chunkToMove.blockCount <= boundary;
        return res;
    }
     ...
     */
    private boolean moveChunk(Chunk chunkNeedMove, long reservedAreaLow, long reservedAreaHigh) {
        if (!chunkId_chunk.containsKey(chunkNeedMove.id)) {
            return false;
        }

        long start = chunkNeedMove.startBlockNumInFile * BLOCK_SIZE;
        int length = chunkNeedMove.blockCount * BLOCK_SIZE;
        ...
        try {
            ...
            writeBuffer.position(chunkHeaderLen);
            writeBuffer.put(readBuff);

            long pos = fileStore.freeSpaceBitSet.allocate(length, reservedAreaLow, reservedAreaHigh);
            blockNum = pos / BLOCK_SIZE;

            // in the absence of a reserved area,
            // block should always move closer to the beginning of the file
            assert reservedAreaHigh > 0 || blockNum <= chunkNeedMove.startBlockNumInFile : blockNum + " " + chunkNeedMove;
            ...
            writeBuffer.position(0);
            chunkFromFile.startBlockNumInFile = blockNum;
            chunkFromFile.nextChunkPosition = 0;
            chunkFromFile.writeChunkHeader(writeBuffer, chunkHeaderLen);

            writeBuffer.position(length - Chunk.FOOTER_LENGTH);
            writeBuffer.put(chunkFromFile.getFooterBytes());

            writeBuffer.position(0);
            write(pos, writeBuffer.getBuffer());
        } finally {
        ...
        chunkNeedMove.startBlockNumInFile = blockNum;
        chunkNeedMove.nextChunkPosition = 0;

        layout.put(Chunk.getMetaKey(chunkNeedMove.id), chunkNeedMove.asString());
     ...
     */
    private void compactUnderTimeLimit(int maxCompactTime) {
        setRetentionTime(0);

        long stopAt = System.nanoTime() + maxCompactTime * 1_000_000L;

        while (rewrite(95, 16 * 1024 * 1024)) {
            sync();

            moveChunks(95, 16 * 1024 * 1024);
     ...
     * @param maxWriteCount  the maxium number of bytes to write
     * @return if a chunk was re-written
     */
    public boolean rewrite(int targetFillRate, int maxWriteCount) {
            ...
            if (targetFillRate > 0 && getChunksFillRate() < targetFillRate) {
                ...
                try {
                    if (storeLock.tryLock(10, TimeUnit.MILLISECONDS)) {
                        try {
                            return rewriteOldChunks(maxWriteCount, 100);
                        } finally {
                            storeLock.unlock();
                        }
                    }
    ...
    private boolean rewriteOldChunks(int maxWriteCount, int targetFillRate) {
        serializationLock.lock();
        ...
        try {
            ...
            try {
                // 收取 deadChunks
                acceptChunkOccupancyChanges(getDurationSinceCreation(), currentVersion);

                // 得到能够rewrite的且fillRate不能达标的chunk的
                Queue<Chunk> oldChunks = findOldChunks(maxWriteCount, targetFillRate);

                if (oldChunks.isEmpty()) {
                    return false;
                }

                HashSet<Integer> oldChunkIds = createIdSet(oldChunks);
                ...
                int rewrittenPageCount = rewritePagesInOldChunks(oldChunkIds, false);

                // 收取 deadChunks
                acceptChunkOccupancyChanges(getDurationSinceCreation(), currentVersion);

                rewrittenPageCount += rewritePagesInOldChunks(oldChunkIds, true);

                return rewrittenPageCount > 0;
            } finally {
    ...
    private int getChunksFillRate(boolean all) {
        ...
        long time = getDurationSinceCreation();
        for (Chunk c : chunkId_chunk.values()) {
    ...
    private Queue<Chunk> findOldChunks(int maxWriteCount, int targetFillRate) {
        assert lastChunk != null;

        long durationSinceCreation = getDurationSinceCreation();
        ...
        PriorityQueue<Chunk> queue = new PriorityQueue<>(chunkId_chunk.size() / 4 + 1,
                (o1, o2) -> {
        ...
        for (Chunk chunk : chunkId_chunk.values()) {
            ...
            // (it's possible to compact chunks earlier, but right now we don't do that)
            if (isRewritable(chunk, durationSinceCreation) && fillRate <= targetFillRate) {
    ...
    private int rewritePagesInOldChunks(Set<Integer> oldChunkIds, boolean secondRound) {
        int rewrittenPageCount = 0;
        for (int chunkId : oldChunkIds) {
            Chunk chunk = chunkId_chunk.get(chunkId);

            long[] toc = getChunkToc(chunk);
            if (toc == null) {
                continue;
            }
            ...
            for (int pageNo = 0; (pageNo = chunk.occupancy.nextClearBit(pageNo)) < chunk.pageCount; ++pageNo) {
                ...
                // page对应mvMap
                int mvMapId = DataUtils.getPageMapId(tocElement);

                MVMap<?, ?> mvMap = mvMapId == layout.getId() ? layout : mvMapId == meta.getId() ? meta : getMap(mvMapId);

                if (mvMap != null && !mvMap.isClosed()) {
                    assert !mvMap.isSingleWriter();

                    // 为了先应对leaf
                    if (secondRound || DataUtils.isLeafPosition(tocElement)) {
                        long pagePos = DataUtils.getPagePos(chunkId, tocElement);

                        serializationLock.unlock();

                        try {
                            if (mvMap.rewritePage(pagePos)) {
                                ++rewrittenPageCount;
     ...
     */
    <K, V> Page<K, V> readPage(MVMap<K, V> mvMap, long position) {
        try {
            if (!DataUtils.isPageSaved(position)) { // position不能是0
                throw DataUtils.newMVStoreException(DataUtils.ERROR_FILE_CORRUPT, "Position 0");
            }

            // 这边的page对应的范型只是函数纬度上的其实是动态的
            Page<K, V> page = readPageFromCache(position);
            if (page == null) {
                Chunk chunk = getChunk(position);
                int pageOffsetInChunk = DataUtils.getPageOffsetInChunk(position);
                try {
                    ByteBuffer byteBuffer = chunk.readBufferForPage(fileStore, pageOffsetInChunk, position);
                    page = Page.readFromByteBuffer(byteBuffer, position, mvMap);
                } catch (MVStoreException e) {
                    throw e;
                } catch (Exception e) {
                    throw DataUtils.newMVStoreException(DataUtils.ERROR_FILE_CORRUPT, "Unable to read the page at position {0}, chunk {1}, offset {2}", position, chunk.id, pageOffsetInChunk, e);
                }
    ...
    private long[] getChunkToc(Chunk chunk) {
        // legacy chunk without table of content
        if (chunk.tocPos == 0) {
            return null;
        }

        long[] toc = chunkTocCache.get(chunk.id);
        if (toc == null) {
            toc = chunk.readToc(fileStore);
            chunkTocCache.put(chunk.id, toc, toc.length * 8);
        }
    ...
    @SuppressWarnings("unchecked")
    private <K, V> Page<K, V> readPageFromCache(long position) {
        return pageCache == null ? null : (Page<K, V>) pageCache.get(position);
    }
    ...
    private int calculatePageNo(long pagePosition) {
        int pageNo = -1;
        Chunk chunk = getChunk(pagePosition);
        long[] toc = getChunkToc(chunk);
        if (toc != null) {
            int pageOffsetInChunk = DataUtils.getPageOffsetInChunk(pagePosition);
            int low = 0;
            int high = toc.length - 1;

            while (low <= high) {
                int mid = (low + high) >>> 1;
                long midVal = DataUtils.getPageOffsetInChunk(toc[mid]);
                if (midVal < pageOffsetInChunk) {
                    low = mid + 1;
                } else if (midVal > pageOffsetInChunk) {
                    high = mid - 1;
                } else {
                    pageNo = mid;
                    break;
     ...
     * We keep at least number of previous versions specified by "versionsToKeep" configuration parameter (default 5).
    ...
    private void setOldestVersionToKeep(long version) {
        while (true) {
            ...
            if (version <= current || oldestVersionToKeep.compareAndSet(current, version)) {
                break;
            }
        }
    ...
    private boolean isKnownVersion(long version) {
        ...
        // no stored data
        if (version == currentVersion || chunkId_chunk.isEmpty()) {
            return true;
        }
        ...
        // also, all chunks referenced by this version need to be available in the file
        ...
        try {
            for (Iterator<String> it = oldLayoutMap.keyIterator(DataUtils.META_CHUNK); it.hasNext(); ) {
                ...
                if (!layout.containsKey(chunkKey)) {
                    ...
                    Chunk test = readChunkHeaderAndFooter(c2.startBlockNumInFile, c2.id);
                    if (test == null) {
                        return false;
                    }
                }
            }
        } catch (MVStoreException e) {  // the chunk missing where the metadata is stored
            return false;
        }
    ...
    public void rollbackTo(long version) {
        ...
        try {
            ...
            if (version == 0) {
                layout.setInitialRootReference(layout.createEmptyLeaf(), INITIAL_VERSION);
                meta.setInitialRootReference(meta.createEmptyLeaf(), INITIAL_VERSION);
                ...
                for (MVMap<?, ?> m : mvMapId_mvMap.values()) {
                    m.close();
                }
            ...
            if (!layout.rollbackRoot(version)) {
                ...
                layout.setInitialRootReference(layoutMap.getRootPage(), version);
            }
            ...
            for (MVMap<?, ?> mvMap : new ArrayList<>(mvMapId_mvMap.values())) {
                ...
                if (mvMap.getCreateVersion() >= version) {
                    mvMap.close();
                    mvMapId_mvMap.remove(id);
                } else {
                    if (!mvMap.rollbackRoot(version)) {
                        mvMap.setRootPos(getRootPos(id), version - 1);
                    }
                }
            }
    ...
    public void renameMap(MVMap<?, ?> map, String newName) {
        checkOpen();
        DataUtils.checkArgument(map != layout && map != meta, "Renaming the meta map is not allowed");
        int id = map.getId();
        ...
        if (oldName != null && !oldName.equals(newName)) {
            ...
            DataUtils.checkArgument(existingIdHexStr == null || existingIdHexStr.equals(idHexStr),
    ...
    public void removeMap(MVMap<?, ?> map) {
        ...
        try {
            checkOpen();
            DataUtils.checkArgument(layout != meta && map != meta, "Removing the meta map is not allowed");
    ...
    void writeInBackground() {
        try {
            ...
            if (getDurationSinceCreation() > lastCommitTime + autoCommitDelay) {
                tryCommit();

                if (autoCompactFillRate < 0) {
                    rewrite(-getTargetFillRate(), autoCommitMemory);
                }
            }

            int fillRate = getFillRate();

            if (fileStore.freeSpaceBitSet.isFragmented() && fillRate < autoCompactFillRate) {
                if (storeLock.tryLock(10, TimeUnit.MILLISECONDS)) {
                    try {
                ...
                if (chunksFillRate < getTargetFillRate()) {
                    if (storeLock.tryLock(10, TimeUnit.MILLISECONDS)) {
                        try {
                            ...
                            if (rewriteOldChunks(writeLimit, chunksFillRate)) {
                                dropUnusedChunks();
                            }
                        } finally {
                            storeLock.unlock();
                ...
                for (int cnt = 0; cnt < 5; cnt++) {
                    ...
                    try {
                        ...
                        if (projectedFillRate < fillRate) {
                            if ((!rewriteOldChunks(writeLimit, targetFillRate) || dropUnusedChunks() == 0) && cnt > 0) {
                                break;
                            }
                        }
                        if (!moveChunks(101, writeLimit)) {
                            break;
                        }
                    } finally {
                        unlockAndCheckPanicCondition();
    ...
    private void stopBackgroundThread(boolean waitForIt) {
        ...
        BackgroundWriterThread backgroundWriterThread0;
        while ((backgroundWriterThread0 = backgroundWriterThread.get()) != null) {
            if (backgroundWriterThread.compareAndSet(backgroundWriterThread0, null)) {
                // if called from within the thread itself - can not join
                if (backgroundWriterThread0 != Thread.currentThread()) {
                    synchronized (backgroundWriterThread0.sync) {
                        backgroundWriterThread0.sync.notifyAll();
                    }

                    if (waitForIt) {
                        try {
                            backgroundWriterThread0.join();
                        } catch (Exception e) {
                            // ignore
                        }
                    }
     ...
     */
    public void setAutoCommitDelay(int maxDelayMs) {
        if (autoCommitDelay == maxDelayMs) {
            return;
        }

        autoCommitDelay = maxDelayMs;
        ...
        if (maxDelayMs > 0 && isOpen()) {
            int sleep = Math.max(1, maxDelayMs / 10);
            BackgroundWriterThread thread = new BackgroundWriterThread(this, sleep, fileStore.toString());

            if (backgroundWriterThread.compareAndSet(null, thread)) {
                thread.start();
    ...
    void cachePage(Page<?, ?> page) {
        if (pageCache != null) {
            pageCache.put(page.position, page, page.getMemory());
        }
    }
    ...
    public int getTocCacheHitRatio() {
        return getCacheHitRatio(chunkTocCache);
    }
    ...
    private void dropUnusedVersions() {
        TxCounter txCounter;
        while ((txCounter = versions.peek()) != null && 0 > txCounter.get()) {
            versions.poll();
        }
    ...
    /**
     * 尚未真正意义上在文件上干掉chunk对应的内容  freeSpaceBitSet chunkId_chunk chunkTocCache pageCache layout 去掉对应的记录
     */
    private int dropUnusedChunks() {
        ...
        if (!deadChunks.isEmpty()) {
            ...
            try {
                Chunk deadChunk;
                while ((deadChunk = deadChunks.poll()) != null &&
                        (isSeasonedChunk(deadChunk, time) && canOverwriteChunk(deadChunk, oldestVersionToKeep) ||
                                // if chunk is not ready yet, put it back and exit 因为 deque 是无限大的, offerFirst()永远是true
                                !deadChunks.offerFirst(deadChunk))) {

                    if (chunkId_chunk.remove(deadChunk.id) != null) {
                        // purge dead pages from cache
                        long[] toc = chunkTocCache.remove(deadChunk.id);
                        if (toc != null && pageCache != null) {
                            for (long tocElement : toc) {
                                long pagePos = DataUtils.getPagePos(deadChunk.id, tocElement);
                                pageCache.remove(pagePos);
                            }
                        }

                        if (layout.remove(Chunk.getMetaKey(deadChunk.id)) != null) {
                            markMetaChanged();
                        }

                        if (deadChunk.isSaved()) {
                            freeFileSpace(deadChunk.startBlockNumInFile * BLOCK_SIZE, deadChunk.blockCount * BLOCK_SIZE);
                        }
    ...
    private static class BackgroundWriterThread extends Thread {
        ...
        BackgroundWriterThread(MVStore mvStore, int sleep, String fileStoreName) {
        ...
        @Override
        public void run() {
            while (mvStore.isBackgroundThread()) {
                synchronized (sync) {
                    try {
                        sync.wait(sleep);
                    } catch (InterruptedException ignore) {
                ...
                if (!mvStore.isBackgroundThread()) {
                    break;
                }

                mvStore.writeInBackground();
            }
        }
    }
    ...
    private static class RemovedPageInfo implements Comparable<RemovedPageInfo> {
        ...
        int getPageNo() {
            return DataUtils.getPageOffsetInChunk(removedPageInfo);
        }
