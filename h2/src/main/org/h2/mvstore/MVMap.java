...
public class MVMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    private K getMinMax(K key, boolean min, boolean excluding) {
        return getMinMax(flushAndGetRootReference(), key, min, excluding);
    }
     ...
     *
     * @param page the root of a snapshot
     * @param key  the key
     * @return the value, or null if not found
     * @throws ClassCastException if type of the specified key is not compatible with this map
     */
    public V get(Page<K, V> page, K key) {
        return Page.get(page, key);
    }
    ...
    RootReference<K, V> clearIt() {
        ...
        while (true) {
            ...
            try {
                ...
                if (isPersistent()) {
                    mvStore.registerUnsavedMemory(rootPage.removeAllRecursive(version));
                }
     ...
     */
    final Page<K, V> readPage(long position) {
        return mvStore.readPage(this, position);
    }
    ...
    public final Cursor<K, V> cursor(K from, K to, boolean reverse) {
        return cursor(flushAndGetRootReference(), from, to, reverse);
    }
    ...
    public final Set<Map.Entry<K, V>> entrySet() {
        final RootReference<K, V> rootReference = flushAndGetRootReference();
    ...
    public final MVStore getMvStore() {
        return mvStore;
    }

    protected final boolean isPersistent() {
        return mvStore.fileStore != null && !isVolatile;
    }
    ...
    private void removeUnusedOldVersions(RootReference<K, V> rootReference) {
        rootReference.removeUnusedOldVersions(mvStore.getOldestVersionToKeep());
    }
    ...
    private void copy(Page<K, V> source, Page<K, V> parent, int index) {
        ...
        mvStore.registerUnsavedMemory(target.getMemory());
        if (mvStore.isSaveNeeded()) {
            mvStore.commit();
        }
    }
            ...
            while ((keyCount = rootReference.getAppendCounter()) > availabilityThreshold) {
                ...
                if (page != null) {
                    ...
                    while (true) {
                        ...
                        if (keyCount <= keysPerPage &&
                                (p.getMemory() < mvStore.getMaxPageSize() || at <= 0)) {
                            break;
                        }
                        key = p.getKey(at);
                        page = p.split(at);
                ...
                if (rootReference != null) {
                    ...
                    if (isPersistent() && tip != null) {
                        mvStore.registerUnsavedMemory(unsavedMemoryHolder.value + tip.processRemovalInfo(version));
                    }
    ...
    public abstract static class DecisionMaker<V> {
         ...
         * @return PUT if a new value need to replace existing one or a new value to be inserted if there is none
